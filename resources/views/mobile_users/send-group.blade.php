                    ...
                    <form id="form" method="post" action="{{ route('send.takons') }}">
                        ...
                        @foreach($users as $user)
                            ...
                            <tr>
                                ...
                                <th>
                                    <button class="btn-danger" id="{{ $user->id }}">Удалить</button>
                                </th>
                                <th>
                                    <select name="service_id[<?=$rand?>]" class="select">
                                        ...
                                        <option value="<?=$service->id?>"
                                                <?php if($service->id == $cs->id) { ?> selected <?php } ?> > <?=$main_serv->name?>
                                            (кол-во: <?=$service->amount?>)
                                        </option>
                                        <?php
                                        }
                                        ?>
                                ...
                                <th>
                                    ...
                                    <input type="number" name="amount[<?=$rand?>]" value="0"
                                           placeholder="Количество таконов" required class="form-control">
                                </th>
                            </tr>
                        @endforeach
